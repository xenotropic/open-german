...
public class Quizzer extends Activity {
	...
	protected void startQuiz (String category) {
		...
		tl_button.setText ("Top Left");
		tr_button.setText ("Top Right");
		bl_button.setText ("Bot Left");
		br_button.setText ("Bot Right");

        // Prevents allcaps due to Material theme that is default API 21+
        tl_button.setTransformationMethod(null);
        tr_button.setTransformationMethod(null);
        bl_button.setTransformationMethod(null);
        br_button.setTransformationMethod(null);
...
//		Log.w ("Quiz setup", "screen width is " + screen_width);
		...
		word_view.setTextColor(Color.rgb( 135, 206, 250 ));
		...
		if (button.getId() != correct_value) {
			...
			button.invalidate();
				if (forward_direction) {
					Toast.makeText(getApplicationContext(), "Correct! " + capitalizeFirstLetter(button.getText().toString()) + " is the translation of " + word_view.getText() + ".", Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(getApplicationContext(), "Correct! " + capitalizeFirstLetter(word_view.getText().toString()) + " is the translation of " + button.getText() + ".", Toast.LENGTH_SHORT).show();
				}
			if (first_answer) {
